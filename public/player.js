...
try{S.playbackRate=parseFloat(localStorage.getItem('alfath_playback_rate'))||1.0;}catch(e){S.playbackRate=1.0;}
try{var storedAutoNext = localStorage.getItem('alfath_auto_next');if(storedAutoNext!==null){S.autoNext = storedAutoNext==='true';}}catch(e){}
...
try {
    var storedVol = parseFloat(localStorage.getItem('alfath_volume'));
...
function applyVolume(vol) {
    ...
    S.volume = vol;
    if (AU) AU.volume = vol;
    try { localStorage.setItem('alfath_volume', String(vol)); } catch(e){}
    updateVolumeUI();
}
...
function toggleAutoNext(){
    ...
    try { localStorage.setItem('alfath_auto_next', S.autoNext); } catch(e) {}
...
function getLikedSongs(){
    try{return JSON.parse(localStorage.getItem('alfath_liked_songs')||'[]');}catch(e){return[];}
}
function saveLikedSongs(songs){
    try{localStorage.setItem('alfath_liked_songs',JSON.stringify(songs));}catch(e){}
}
...
function getLikedArtists(){
    try{return JSON.parse(localStorage.getItem('alfath_liked_artists')||'[]');}catch(e){return[];}
}
function saveLikedArtists(artists){
    try{localStorage.setItem('alfath_liked_artists',JSON.stringify(artists));}catch(e){}
}
...
function getUserPlaylists(){
    try{
        var pls=JSON.parse(localStorage.getItem('alfath_playlists')||'[]');
        var changed=false;
        ...
        if(changed){
            localStorage.setItem('alfath_playlists',JSON.stringify(pls));
        }
        return pls;
    }catch(e){return[];}
}
function saveUserPlaylists(pls){try{localStorage.setItem('alfath_playlists',JSON.stringify(pls));}catch(e){}}
...
function setPlaybackSpeed(speed) {
    ...
    try {
        localStorage.setItem('alfath_playback_rate', speed);
    } catch(e) {}
...
function downloadShareCard() {
    ...
    img.onload = function() {
        ...
        try {
            ...
            a.download = S.ct.title.replace(/[^a-zA-Z0-9]/g, '_') + '_alfath.png';
            a.href = dataUrl;
            a.click();
